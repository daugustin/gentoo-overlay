...
# 2.x above every 1.x, so the bump needs no revision gymnastics. 2.2553.x is
# ordered above 2.110.0 the same way, field by field (2553 > 110), and .13
# above .1 within the series.
#
# 2.2553.13 is an automated upstream bump. Between v2.2553.1 and v2.2553.13
# the repackaging tree only gained Nix fixes, README text and a colour theme,
# none of which reach the Linux tarball, and upstream's patch set re-applied
# unchanged. launcher/claude-desktop is byte-identical to 2.2553.1's, and the
# tarball keeps the same 3622 entries, modes and sizes on the same bundled
# Electron 44.2.0 -- resources/app.asar and the content-hashed ion-dist
# chunks are the only files that differ.
