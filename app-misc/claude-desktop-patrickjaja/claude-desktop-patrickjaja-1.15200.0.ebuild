...
src_install() {
	...
	# identity / generated .desktop filename), so the Electron binary is
	# renamed to match. The window WM_CLASS / Wayland app_id is instead
	# "claude-desktop", derived from the app's desktopName in app.asar and
	# not from the binary basename -- the installed .desktop's
	# StartupWMClass matches that.
