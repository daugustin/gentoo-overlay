...
IUSE="openblas +openmp blis rocm cuda opencl openssl vulkan flexiblas examples"
...
src_configure() {
	local mycmakeargs=(
		# Upstream renamed LLAMA_BUILD_WEBUI to LLAMA_BUILD_UI (May 2026) with no
		# deprecation alias. Disabling the UI also requires LLAMA_USE_PREBUILT_UI=OFF,
		# otherwise cmake still downloads the prebuilt UI from the HF Bucket, which
		# fails in the network-isolated sandbox.
		-DLLAMA_BUILD_UI=OFF
...
pkg_postinst() {
	...
	for v in ${REPLACING_VERSIONS}; do
		if ver_test "${v}" -lt "0_pre10453"; then
			elog "Upstream dropped the rocWMMA FlashAttention code path"
			elog "(llama.cpp#26046), so the 'wmma' USE flag is gone."
		fi
		if ver_test "${v}" -lt "0_pre9941"; then
			elog "Upstream renamed the RPC server binary:"
			elog "  rpc-server -> ggml-rpc-server"
		fi
		break
	done
}
