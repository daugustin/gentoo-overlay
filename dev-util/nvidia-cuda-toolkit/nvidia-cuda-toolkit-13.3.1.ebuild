...
cuda-toolkit_check_reqs() {
	if use amd64; then
		local CHECKREQS_DISK_BUILD="7195M"
	elif use arm64; then
		local CHECKREQS_DISK_BUILD="6969M"
	fi
...
src_install() {
	...
	# Add lib symlink; since CUDA 13 the top-level include symlink is
	# installed via the manifest itself
