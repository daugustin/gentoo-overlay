...
WXVERSIONTAG=$(ver_cut 1-2)				# 3.3
WXRELEASE="${WXVERSIONTAG}-gtk3"		# 3.3-gtk3
...
multilib_src_configure() {
	...
	# By default, we build with the GLX GLCanvas because some software like
	# PrusaSlicer does not support EGL:
	#
...
multilib_src_install_all() {
	...
	mv "${ED}"/usr/share/aclocal/wxwin.m4 \
		"${ED}"/usr/share/aclocal/wxwin${WXVERSIONTAG/./}-gtk3.m4 || die

	# version bakefile presets
	pushd "${ED}"/usr/share/bakefile/presets >/dev/null || die
	local f
	for f in wx*; do
		mv "${f}" "${f/wx/wx${WXRELEASE//-/}}" || die
	done
	popd >/dev/null || die
}
